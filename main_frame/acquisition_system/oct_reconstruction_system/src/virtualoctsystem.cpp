...
VirtualOCTSystem::VirtualOCTSystem() {
    ...
    this->name = "Reconstruction OCT System";
	this->file = nullptr;
...
VirtualOCTSystem::~VirtualOCTSystem() {
	this->cleanup();
    qDebug() << "Reconstruction destructor. Thread ID: " << QThread::currentThreadId();
}
...
void VirtualOCTSystem::acqcuisitionSimulation(){
    ...
    while (this->acquisitionRunning) {
        ...
        while(this->buffer->bufferReadyArray[buffer->currIndex] == true && this->acquisitionRunning){
			QThread::usleep(100);
			QCoreApplication::processEvents();
		}
...
void VirtualOCTSystem::acquisitionSimulationWithMultiFileBuffers() {
	...
	size_t bufferSizeInBytes = numberOfElements*sizeOfElement;
    fclose(this->file);
    QFile file(this->currParams.filePath.toLatin1());
	...
	for(int i = 0; i < currParams.buffersFromFile; i++){
        ...
        file.seek(i * bufferSizeInBytes);
        auto data = file.read(bufferSizeInBytes);
        qDebug() << data.size();
        memcpy(buf, data.constData(), bufferSizeInBytes);
	}
    qDebug() << "retrurn";
    file.close();
...
void VirtualOCTSystem::slot_updateParams(simulatorParams newParams){
    ...
    params.procFilePath = newParams.paraPath;
    params.samplesPerAline = newParams.width;
    params.alinesPerBscan = newParams.height;
    ...
    params.lengthX = newParams.xRange;
    params.lengthY = newParams.yRange;
    params.lengthZ = newParams.maxZRange;
