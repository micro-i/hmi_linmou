...
public slots:
    ...
    virtual void rawDataReceived(void* buffer, unsigned int bitDepth, unsigned int samplesPerLine, unsigned int alinePerBscan, unsigned int bscanPerBuffer, unsigned int buffersPerVolume, unsigned int currentBufferNr);
    // 处理后的数据可用时调用
    virtual void processedDataReceived(void* buffer, unsigned int bitDepth, unsigned int samplesPerLine, unsigned int alinePerBscan, unsigned int bscanPerBuffer, unsigned int buffersPerVolume, unsigned int currentBufferNr);
...
#define Extension_iid "UHROCT.extension.interface"
